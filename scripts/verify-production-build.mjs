...
if(migration.schema_version!==1||!Array.isArray(migration.stages)||migration.stages.length!==9||!Array.isArray(migration.gates))throw new Error('Le suivi de migration de production est incomplet.')
if(migration.cutover_ready&&!migration.gates.every(item=>item.passed))throw new Error('Le basculement est déclaré prêt sans validation des prérequis.')
...
if(!serviceWorker.includes("const CACHE_NAME = 'danz-shell-v25'"))throw new Error('La version attendue du cache PWA v25 est absente.')
