...
const required = [
  ...
  'icon-192-v8.png',
  'icon-512-v8.png',
  'icon-maskable-192-v8.png',
  'icon-maskable-512-v8.png',
]
...
async function assertFile(relativePath) {
  ...
  if (!info.isFile() || info.size === 0) throw new Error(`Fichier de production invalide : ${relativePath}`)
}
...
for (const icon of manifest.icons || []) {
  const pathname = String(icon.src || '').split('?')[0].replace(/^\/danz\//, '')
  if (!pathname) throw new Error('Une icône PWA n’a pas de chemin valide.')
  await assertFile(pathname)
}

if (!serviceWorker.includes("const CACHE_NAME = 'danz-shell-v22'")) throw new Error('La version attendue du cache PWA v22 est absente.')
