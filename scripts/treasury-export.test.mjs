...
test('un export Excel contient les 11 feuilles et toutes les écritures, même sans solde initial', () => {
  const sheets = buildFinancialSheets({
    ...
    charges: [{ id: 'c1', household_id: 'h1', offline_person_id: 'o1', status: 'open', category: 'membership', label: 'Cotisation',
  ...
  assert.equal(Object.keys(zip).filter((f) => /^xl\/worksheets\/sheet\d+.xml$/.test(f)).length, 11)
