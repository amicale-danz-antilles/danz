...
export default function Dashboard(){
 ...
 const openDetail=async(kind,item)=>{
  setDetail({kind,item:{...item},loadingExtra:kind==='news'})
  if(kind!=='news')return
  try{
 ...
 return <div className="home-dashboard home-dashboard-compact home-app-dashboard">
  ...
  {detail&&<div className="home-detail-backdrop" role="presentation" onClick={()=>setDetail(null)}><section className="home-detail-modal" role="dialog" aria-modal="true" aria-label={detail.item.title} onClick={e=>e.stopPropagation()}><button type="button" className="home-detail-close" aria-label="Fermer" onClick={()=>setDetail(null)}>×</button>{detail.item.cover&&<img className="home-detail-cover" src={detail.item.cover} alt="" decoding="async"/>}<div className="home-detail-content"><span className="eyebrow">{detail.kind==='news'?'Actualité':new Date(detail.item.starts_at).getTime()>=Date.now()?'Événement à venir':'Événement récent'}</span><h2>{detail.item.title}</h2>{detail.kind==='news'?<><time>{formatDate(detail.item.publish_at||detail.item.published_at)}</time>{detail.item.content&&<p className="home-detail-text">{detail.item.content}</p>}{detail.loadingExtra&&detail.item.assets?.some(a=>!a.is_cover)&&<p className="home-detail-loading">Chargement des pièces jointes…</p>}{!detail.loadingExtra&&detail.item.assets?.filter(a=>!a.is_cover).length>0&&<div className="home-detail-files"><strong>Pièces jointes</strong>{detail.item.assets.filter(a=>!a.is_cover).map(a=><a key={a.id} href={a.url||'#'} target="_blank" rel="noopener noreferrer">📎 {a.file_name}</a>)}</div>}</>:<><p className="home-detail-meta">📅 {formatDate(detail.item.starts_at)} · {new Date(detail.item.starts_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</p>{detail.item.location&&<p className="home-detail-meta">📍 {detail.item.location}</p>}{detail.item.description&&<p className="home-detail-text">{detail.item.description}</p>}{detail.item.album&&<><div className="home-album-summary"><span>📷</span><div><strong>Album disponible</strong><small>{detail.item.album.item_count!=null?`${detail.item.album.item_count} média${detail.item.album.item_count>1?'s':''} à télécharger`:'Téléchargement et souvenirs de l’événement'}</small></div></div><Link className="primary-button home-detail-album-link" to={`/galerie?event=${detail.item.id}`} onClick={()=>setDetail(null)}>Ouvrir l’album →</Link></>}</>}</div></section></div>}
 </div>
}
