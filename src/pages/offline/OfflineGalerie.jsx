...
export default function OfflineGalerie(){
  ...
  return <>
    ...
    <div className="offline-v2-notice"><strong>Index hors ligne</strong><span>{entry?.savedAt?`Copie synchronisée le ${new Date(entry.savedAt).toLocaleString('fr-FR')}. `:''}Les liens WeTransfer ne sont pas conservés localement.</span></div>
    {!entry?<div className="empty-state">Aucun album n’a encore été synchronisé sur cet appareil. Ouvrez la Galerie une fois avec Internet pour préparer le mode hors ligne.</div>:selected?<AlbumDetail album={selected} onBack={()=>setSelectedId('')}/>:<div className="gallery-album-grid offline-album-grid">{albums.length?albums.map(album=><button className="gallery-album-card" type="button" key={album.id} onClick={()=>setSelectedId(album.id)}><div className="gallery-album-cover">{album.offline_thumb?<img src={album.offline_thumb} alt=""/>:<span>📷</span>}{album.item_count!=null&&<span className="gallery-album-count">{album.item_count} média{album.item_count>1?'s':''}</span>}</div><div className="gallery-album-info"><strong>{album.event?.title||'Album'}</strong><span>{formatDate(album.event?.starts_at)}</span>{album.event?.location&&<span>📍 {album.event.location}</span>}<small>Ouvrir la fiche →</small></div></button>):<div className="empty-state">Aucun album enregistré.</div>}</div>}
  </>
}
