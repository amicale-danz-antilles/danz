...
import { getR2Status, removePrivateMedia, uploadPrivateMedia } from '../lib/mediaStorage.js'
...
export default function AdminContent(){
 ...
 const validatePhoto=(file)=>{if(!file)return;if(!file.type.startsWith('image/'))throw new Error('La photo principale doit être une image.');if(file.size>PHOTO_LIMIT)throw new Error('La photo principale dépasse 20 Mo.')}
 ...
 const addAttachment=async(file,parentType,parentId,isCover=false)=>{
  ...
  const payload={
   news_id:parentType==='news'?parentId:null,event_id:parentType==='events'?parentId:null,
   file_name:file.name||'fichier',storage_provider:stored.storage_provider,storage_path:stored.storage_path,
   mime_type:file.type||null,file_size:file.size,is_cover:isCover,created_by:user.id,
  }
  ...
  try{
   ...
   if(type==='news'){
    ...
    const result=editing
      ? await supabase.from('news').update(payload).eq('id',editing.id).select('id').single()
      : await supabase.from('news').insert({...payload,published_at:publicationIso,created_by:user.id}).select('id').single()
    ...
    if(resetNotification)payload.notified_at=null
    const result=editing
      ? await supabase.from('events').update(payload).eq('id',editing.id).select('id').single()
      : await supabase.from('events').insert({...payload,created_by:user.id}).select('id').single()
    if(result.error)throw result.error;parentId=result.data.id
   }

   if(cover)await addAttachment(cover,type,parentId,true)
   if(type==='news'&&files.length)for(const file of files)await addAttachment(file,'news',parentId,false)
   setSuccess(`${type==='news'?'Actualité':'Événement'} ${editing?'modifié':'publié'}${r2Ready?' avec stockage R2 pour les nouveaux fichiers':''}.`)
 ...
 const removeItem=async(item)=>{
  ...
  for(const asset of attachments||[])await removePrivateMedia(asset,{fallbackBucket:'content'})
 ...
 return <>
  <PageTitle eyebrow="Administration" title="Publier" text="Publiez simplement une actualité ou un événement. Photos et pièces jointes sont rattachées directement à la publication."/>
  <div className="text-panel">
   <form onSubmit={submit}>
    ...
    {type==='events'&&<>
      <label>Lieu<input maxLength="180" value={location} onChange={e=>setLocation(e.target.value)}/></label>
      <label>Date et heure<input type="datetime-local" required value={startsAt} onChange={e=>setStartsAt(e.target.value)}/></label>
      <label>Fin (facultatif)<input type="datetime-local" value={endsAt} onChange={e=>setEndsAt(e.target.value)}/></label>
    </>}
    <label>Photo principale (facultatif)<input id="content-cover" type="file" accept="image/*" onChange={e=>setCover(e.target.files?.[0]||null)}/></label>
    ...
    <details className="good-deals-advanced"><summary>Options avancées</summary><div style={{display:'grid',gap:'1rem',marginTop:'1rem'}}>
      <label>Audience<select value={audience} onChange={e=>setAudience(e.target.value)}><option value="everyone">Tout le monde</option><option value="military">Militaires DANZ uniquement</option><option value="amicaliste">Amicalistes uniquement</option><option value="admin">Bureau / Admin uniquement</option></select></label>
      <label>Publication<select value={schedule} onChange={e=>setSchedule(e.target.value)}>{editing&&<option value="keep">Conserver la publication actuelle</option>}<option value="now">Publier maintenant</option><option value="later">Programmer pour plus tard</option></select></label>
      {schedule==='later'&&<label>Date de publication<input type="datetime-local" required value={publishAt} onChange={e=>setPublishAt(e.target.value)}/></label>}
    </div></details>
    <div className="privacy-note">☁️ {r2Ready?'Cloudflare R2 est connecté pour les nouveaux fichiers.':'Cloudflare R2 est prêt côté site mais pas encore raccordé aux clés Cloudflare. Supabase sert de stockage de secours.'}</div>
