...
export default function Profile() {
  ...
  if (!isAdmin) {
    ...
    memberTags.push(profile?.is_amicaliste === true ? 'Amicaliste' : 'Non-amicaliste')
  }

  return <>
    <PageTitle eyebrow="Compte" title="Mon profil" text="Gérez votre accès, votre mot de passe et vos préférences de notifications." />
    <div className="profile-card"><div className="profile-avatar">{(profile?.full_name || user.email || 'A')[0].toUpperCase()}</div><div><h2>{profile?.full_name || roleLabel}</h2><p>{user.email}</p><div style={{display:'flex',gap:'.4rem',flexWrap:'wrap'}}><span className="role-badge">{roleLabel}</span>{memberTags.map((tag) => <span className="role-badge" key={tag}>{tag}</span>)}</div></div></div>
    ...
    <div className="text-panel"><h2>Publications à notifier</h2><p>Choisissez les catégories pour lesquelles vous souhaitez recevoir une notification.</p><div style={{ display: 'grid', gap: '.8rem', marginTop: '1rem' }}>{Object.entries(preferenceLabels).map(([key, label]) => <label key={key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '.85rem 0', borderBottom: '1px solid rgba(20,55,65,.1)' }}><span><strong>{label}</strong></span><input type="checkbox" checked={preferences[key]} onChange={() => togglePreference(key)} style={{ width: '1.25rem', height: '1.25rem', flex: '0 0 auto' }} /></label>)}</div></div>

    <div className="text-panel"><h2>Mot de passe</h2><p>Membres et administrateurs utilisent le même mode de connexion : adresse e-mail + mot de passe. Vous pouvez modifier votre mot de passe ici sans changer vos droits ni votre statut amicaliste.</p><form onSubmit={updatePassword}><label>Nouveau mot de passe<input type="password" minLength="10" required autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} /></label><label>Confirmer le mot de passe<input type="password" minLength="10" required autoComplete="new-password" value={confirm} onChange={(e) => setConfirm(e.target.value)} /></label><button className="primary-button" disabled={busy}>{busy ? 'Enregistrement…' : 'Enregistrer le mot de passe'}</button></form></div>
  </>
}
