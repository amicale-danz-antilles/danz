...
const method = { bank_transfer: 'Compte bancaire / virement', card: 'Compte bancaire / carte', cash: 'Caisse (liquide)', personal_advance: 'Avance personnelle', unassigned: 'À ventiler (origine Excel)' }
...
export function buildFinancialSheets(data, takenAt = new Date()) {
  const { entries = [], transfers = [], households = [], members = [], profiles = [], offline = [], charges = [], payments = [], allocations = [], subscriptions = [], opening = null, importBatches = [], importArchive = [] } = data
  ...
  const sheets = [
    { name: 'Synthèse', headers: ['Indicateur', 'Valeur', 'Commentaire'], rows: [
      ...
      ['Total comptable', balances ? eur(Math.round(balances.bank + balances.cash + balances.unassigned)) : 'Non initialisé', 'Banque + caisse + mouvements à ventiler ; hors créances et avances'],
    ...
    { name: 'Soldes et référence', headers: ['Compte ou référence', 'Montant EUR', 'Date', 'Note'], rows: [
      ...
      ['Total comptable', balances ? eur(balances.bank + balances.cash + balances.unassigned) : 'Non initialisé', asDate(takenAt), 'Somme des deux comptes et du non ventilé'],
    ] },
