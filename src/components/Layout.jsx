...
export default function Layout() {
  const { user, profile, isAdmin, isTreasurer, signOut } = useAuth()
  ...
  useEffect(() => { setOpen(false); window.scrollTo({ top: 0, left: 0, behavior: 'auto' }) }, [location.pathname])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onComplete = (event) => {
      refresh(); const synced = Number(event.detail?.synced || 0); const failed = Number(event.detail?.failed || 0)
    ...
    refresh(); window.addEventListener('danz-offline-queue-changed', refresh); window.addEventListener('danz-offline-sync-complete', onComplete)
    return () => { window.removeEventListener('danz-offline-queue-changed', refresh); window.removeEventListener('danz-offline-sync-complete', onComplete) }
  }, [user?.id])
  useEffect(() => {
  ...
  const logout = async () => {
    ...
    if (user?.id) { await clearOfflineMutations(user.id).catch(() => {}); clearOfflineData(user.id) }
    await detachPushSubscription(); await signOut(); navigate('/connexion')
  }
  ...
  memberDetails.push(effectiveAmicaliste(profile) ? 'Amicaliste' : 'Non-amicaliste')
  const spaceTitle = isTreasurer ? 'Espace administrateur · Trésorier' : isAdmin ? 'Espace administrateur' : 'Espace membre'
  const spaceSubtitle = isAdmin ? ['Administration', ...memberDetails].join(' · ') : memberDetails.join(' · ') || 'Compte membre validé'
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? 'open' : ''}`} aria-label="Navigation principale">
      ...
      <nav>{links.map(([to, label, icon]) => <NavLink key={to} to={to} end={to === '/'} onClick={() => setOpen(false)}><span className="nav-icon">{icon}</span>{label}</NavLink>)}{isTreasurer && <NavLink to="/administration/tresorerie" onClick={() => setOpen(false)}><span className="nav-icon">€</span>Trésorerie</NavLink>}{isAdmin && <NavLink to="/administration" onClick={() => setOpen(false)}><span className="nav-icon">⚙</span>Administration</NavLink>}</nav>
      <div className="sidebar-footer"><NavLink className="member-chip profile-chip-link" to="/profil" onClick={() => setOpen(false)} title="Ouvrir mon profil"><span>{(profile?.full_name || user?.email || 'A')[0].toUpperCase()}</span><small><strong>{profile?.full_name || user?.email}</strong><em>Mon profil</em></small></NavLink><NavLink to="/confidentialite" onClick={() => setOpen(false)} style={{ fontSize: '11px', color: 'var(--muted)', textAlign: 'center' }}>Confidentialité & RGPD</NavLink><button className="ghost-button" onClick={logout}>Se déconnecter</button></div>
    </aside>
