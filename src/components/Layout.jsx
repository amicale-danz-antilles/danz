...
export default function Layout() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? 'open' : ''}`} aria-label="Navigation principale">
      ...
      <nav>{links.map(([to, label, icon]) => <NavLink key={to} to={to} end={to === '/'} onClick={() => setOpen(false)}><span className="nav-icon">{icon}</span>{label}</NavLink>)}{isAdmin && <NavLink to="/administration" onClick={() => setOpen(false)}><span className="nav-icon">⚙</span>Administration</NavLink>}</nav>
