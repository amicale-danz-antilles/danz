    ...
    const timer = window.setTimeout(async () => {
      try {
        const [dealsResult, pollsResult, albumsResult, eventsResult] = await Promise.all([
          ...
          supabase.from('events').select('id,title,description,location,starts_at,ends_at,audience').order('starts_at', { ascending: true }),
        ])
        if (cancelled) return
        ...
        if (!albumsResult.error && !eventsResult.error) {
          ...
          saveOfflineData(userId, 'agenda', { items: events, albums: agendaAlbums })
        } else if (!eventsResult.error) {
          saveOfflineData(userId, 'agenda', { items: events, albums: {} })
        }
